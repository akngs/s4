expect.extend({
  toContainInOrder(received: unknown, parts: string[]) {
    if (typeof received !== "string") return { pass: false, message: () => `toContainInOrder: expected a string but received ${typeof received}` }
    ...
    for (let i = 1; i < indices.length; i += 1) {
      ...
      if (cur < prev) {
        ...
        return { pass: false, message: () => `toContainInOrder: substrings out of order at index ${i}. Expected non-decreasing indices.\n${report}` }
      }
    }

    // 3. Success
    return { pass: true, message: () => "toContainInOrder: expected substrings not to appear in order" }
  },
  toBeError(received: { exitCode: number; stderr: string }, expectedType: string) {
    ...
    return {
      pass,
      message: () =>
        `toError: expected exitCode=1 and stderr to contain "${expectedType}" but got exitCode=${received.exitCode}, stderr=${JSON.stringify(received.stderr)}`,
    }
  },
})
